...
<div class="grid md:grid-cols-2 gap-6">
    ...
    <div>
        <label class="block font-semibold text-gray-700 mb-2">أيقونة (Font Awesome class)</label>
        <input type="text" name="icon" value="{{ old('icon', $value->icon ?? '') }}" placeholder="fa-solid fa-star"
               class="w-full rounded-xl border border-gray-300 focus:border-secondary focus:ring-2 focus:ring-secondary/30 px-4 py-3" dir="ltr">
...
<div class="mt-6">
    ...
    <textarea name="description" rows="3"
              class="w-full rounded-xl border border-gray-300 focus:border-secondary focus:ring-2 focus:ring-secondary/30 px-4 py-3">{{ old('description', $value->description ?? '') }}</textarea>
</div>

<div class="grid md:grid-cols-2 gap-6 mt-6">
    <div>
        <label class="block font-semibold text-gray-700 mb-2">الترتيب</label>
        <input type="number" name="order" value="{{ old('order', $value->order ?? 0) }}"
               class="w-full rounded-xl border border-gray-300 focus:border-secondary focus:ring-2 focus:ring-secondary/30 px-4 py-3">
    </div>
    <div class="flex items-center gap-3 pt-8">
        <input type="checkbox" name="is_active" value="1" id="is_active"
               {{ old('is_active', $value->is_active ?? true) ? 'checked' : '' }}
...
<div class="mt-8">
    ...
    <a href="{{ route('admin.core-values.index') }}" class="text-gray-500 font-semibold px-4">إلغاء</a>
</div>
